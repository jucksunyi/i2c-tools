...
int main(int argc, char *argv[])
{
	...
	MYDEBUG("i2cbus: %d flags :%d, argc :%d", i2cbus, flags, argc);
	if (argc < flags + 4)
		MYDEBUG("size = I2C_SMBUS_BYTE_DATA");
	...
	if (bank && size != I2C_SMBUS_BLOCK_DATA)
		MYDEBUG("	if (bank && size != I2C_SMBUS_BLOCK_DATA)");
	else
		MYDEBUG("	if (bank && size != I2C_SMBUS_BLOCK_DATA) ERROR");
	...
	if (size != I2C_SMBUS_WORD_DATA || even)
		MYDEBUG("	if (size != I2C_SMBUS_WORD_DATA || even) ");	
	else
		MYDEBUG("	if (size != I2C_SMBUS_WORD_DATA || even) ERROR");

	MYDEBUG("size: 0x%02x", size);
	MYDEBUG("i2cfirst: 0x%02x, i2clast: 0x%02x", first, last);
	...
	for(tmp = 0; tmp <= sizeof(i2c_tmp_buf); tmp++)
	{
		if(i2c_tmp_buf[tmp] != 0)
		{
			MYDEBUG("i2c_tmp_buf[%02x] : 0x%02x", tmp, i2c_tmp_buf[tmp]);
		}
	}
